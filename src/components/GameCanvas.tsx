...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha, sidewalkTexture, sidewalkScale, sidewalkAlpha, sidewalkTint }) => {
    ...
    const renderSidewalkRing = ({
        ...
        let outerLoop = sanitizeLoopPoints(worldPts);
        if (outerLoop.length < 3) return;

        if (polygonSignedArea(outerLoop) < 0) {
            outerLoop = outerLoop.slice().reverse();
        }

        let insetLoop = insetPolygonByDistance(outerLoop, widthM);
        if (insetLoop && insetLoop.length >= 3 && polygonSignedArea(insetLoop) < 0) {
        ...
        if (outerLoop.length >= 3 && insetLoop && insetLoop.length === outerLoop.length) {
            let outerPath = outerLoop.map(p => ({ X: Math.round(p.x * clipScale), Y: Math.round(p.y * clipScale) }));
            if (!ClipperLib.Clipper.Orientation(outerPath)) {
                outerPath = outerPath.slice().reverse();
            }
