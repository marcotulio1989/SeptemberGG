...
const generateRoadCrackContours = ({
    ...
    const dominantIsoSpan = Math.max(isoSpanX, isoSpanY);
    const epsilonNormalized = epsilonPx * Math.SQRT2 / Math.max(dominantIsoSpan, 1e-5);
    ...
    while (normalizedSeeds.length < targetSeeds * 2 && attempts < maxAttempts) {
        ...
        const relX = wx - baseX;
        const relY = wy - baseY;
        const alongNorm = clamp(relX * ux + relY * uy, -1e-6, length + 1e-6) / Math.max(length, 1e-6);
        const lateralNorm = 0.5 + (relX * nx + relY * ny) / Math.max(width, 1e-6);
        normalizedSeeds.push(clamp(alongNorm, 0, 1), clamp(lateralNorm, 0, 1));
    }
    const actualSeeds = normalizedSeeds.length / 2;
    ...
    const cellNormU = 1 / bucketCols;
    const cellNormV = 1 / bucketRows;
    const buckets: number[][] = new Array(bucketCols * bucketRows);
    for (let i = 0; i < buckets.length; i++) buckets[i] = [];
    for (let i = 0; i < actualSeeds; i++) {
        const normU = normalizedSeeds[2 * i];
        const normV = normalizedSeeds[2 * i + 1];
        if (!Number.isFinite(normU) || !Number.isFinite(normV)) continue;
        const gx = Math.min(bucketCols - 1, Math.max(0, Math.floor(Math.min(normU, 0.999999) / cellNormU)));
        const gy = Math.min(bucketRows - 1, Math.max(0, Math.floor(Math.min(normV, 0.999999) / cellNormV)));
    ...
    for (let jy = 0; jy < gridH; jy++) {
        ...
        for (let ix = 0; ix < gridW; ix++) {
            ...
            const normAlong = clamp(along / Math.max(length, 1e-6), 0, 1);
            const normAcross = clamp(0.5 + lateral / Math.max(width, 1e-6), 0, 1);
            const gx = Math.min(bucketCols - 1, Math.max(0, Math.floor(Math.min(normAlong, 0.999999) / cellNormU)));
            const gy = Math.min(bucketRows - 1, Math.max(0, Math.floor(Math.min(normAcross, 0.999999) / cellNormV)));
            for (let r = 1; r <= 2; r++) {
            ...
            for (let k = 0; k < source.length; k++) {
                const idx = source[k];
                const diffU = normAlong - normalizedSeeds[2 * idx];
                const diffV = normAcross - normalizedSeeds[2 * idx + 1];
                const dist2 = diffU * diffU + diffV * diffV;
                if (dist2 < best1) {
                    best2 = best1;
                    best1 = dist2;
                } else if (dist2 < best2) {
            ...
            mask[jy * gridW + ix] = delta < epsilonNormalized ? 1 : 0;
        }
    }

    const isoContours = marchingSquaresContoursLocal(mask, gridW, gridH, stepIsoX, stepIsoY, isoMinX, isoMinY);
    const worldContours: LocalPoint[][] = [];
    for (const contour of isoContours) {
        if (!contour || contour.length < 2) continue;
        const poly: LocalPoint[] = [];
        let valid = true;
        for (const pt of contour) {
            const w = isoToWorld({ x: pt[0], y: pt[1] });
