    ...
    const renderSidewalkRing = ({
        ...
        if (walkwayQuads.length) {
            for (const quad of walkwayQuads) {
                const pts = quad.map(p => worldToIso(p));
                if (pts.length > 2) {
                    ringG.drawPolygon(pts.flatMap(pt => [pt.x, pt.y]));
                }
            }
        } else if (clipperRingPaths && clipperRingPaths.length) {
            for (const path of clipperRingPaths) {
                const pts = path.map((p: any) => worldToIso({ x: p.X / clipScale, y: p.Y / clipScale }));
                if (pts.length > 2) {
