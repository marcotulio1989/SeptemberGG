...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha, sidewalkTexture, sidewalkScale, sidewalkAlpha, sidewalkTint }) => {
    ...
    const renderSidewalkRing = ({
        ...
        if (insetLoop && insetLoop.length >= 3) {
            const alignedInner = rotateLoopToClosestPoint(ensureLoopOrientation(insetLoop, true), outerLoop[0]);
            ringsForTiles.push({
        ...
        try {
            ...
            if (innerPaths && innerPaths.length) {
                ...
                const ringTileCandidates: { outer: Point[]; inner: Point[] }[] = [];
                    ...
                    if (contour && contour.length > 2 && childs && childs.length) {
                        ...
                        for (let i = 0; i < childs.length; i++) {
                            ...
                            if (holeContour && holeContour.length > 2) {
                                ...
                                const insetCandidate = insetPolygonByDistance(outerWorld, widthM);
                                const innerForTiles = (insetCandidate && insetCandidate.length >= 3)
                                    ? ensureLoopOrientation(insetCandidate, true)
                                    : innerWorld;
                                ringTileCandidates.push({
                                    outer: outerWorld.map(pt => ({ x: pt.x, y: pt.y })),
                                    inner: rotateLoopToClosestPoint(innerForTiles, outerWorld[0]),
                                });
                            }
                            gatherRings(child);
                        }
                ...
                if (!ringsForTiles.length && ringTileCandidates.length) {
                    for (const candidate of ringTileCandidates) {
                        if (candidate.outer.length >= 3 && candidate.inner.length >= 3) {
                            ringsForTiles.push({
                                outer: candidate.outer.map(pt => ({ x: pt.x, y: pt.y })),
                                inner: candidate.inner.map(pt => ({ x: pt.x, y: pt.y })),
                            });
                        }
                    }
                }
        ...
        if (!ringsForTiles.length && insetLoop && insetLoop.length >= 3) {
            ringsForTiles.push({
                outer: outerLoop.map(pt => ({ x: pt.x, y: pt.y })),
                inner: rotateLoopToClosestPoint(ensureLoopOrientation(insetLoop, true), outerLoop[0]),
            });
        }
