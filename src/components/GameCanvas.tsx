...
const generateRoadCrackSprite = ({
    ...
    const hardThreshold = Math.max(0.001, Math.min(isoThreshold, isoThreshold - feather + 1e-6));
    ...
    let attempts = 0;
    while (localSeeds.length < targetSeeds * 2 && attempts < maxAttempts) {
        attempts++;
    ...
    const actualSeeds = localSeeds.length / 2;
    ...
    const spanAlong = Math.max(1e-3, length);
    const spanAcross = Math.max(1e-3, width);
    const cellAlong = Math.max(spanAlong / gridCols, spanAlong / (actualSeeds || 1));
    const cellAcross = Math.max(spanAcross / gridRows, spanAcross / (actualSeeds || 1));
    const buckets: number[][] = new Array(gridCols * gridRows);
    for (let i = 0; i < buckets.length; i++) buckets[i] = [];
    for (let i = 0; i < actualSeeds; i++) {
        const along = localSeeds[2 * i];
        const lateral = localSeeds[2 * i + 1];
        if (!Number.isFinite(along) || !Number.isFinite(lateral)) continue;
        const gx = Math.min(gridCols - 1, Math.max(0, Math.floor(along / cellAlong)));
        const gyRaw = (lateral + halfWidth) / Math.max(cellAcross, 1e-3);
        const gy = Math.min(gridRows - 1, Math.max(0, Math.floor(gyRaw)));
    ...
    for (let py = 0; py < pixelRows; py++) {
        ...
        for (let px = 0; px < pixelCols; px++) {
            ...
            for (let k = 0; k < source.length; k++) {
                const idx = source[k];
                const dx = along - localSeeds[2 * idx];
                const dy = lateral - localSeeds[2 * idx + 1];
