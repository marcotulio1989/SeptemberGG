...
export const generateIsometricTilePattern = (
    ...
    patternCanvas.height = H + thickness;
    const patternCtx = patternCanvas.getContext('2d');
    if (!patternCtx) return null;

    const backgroundColor =
        partialOptions.backgroundColor === null
            ? null
            : partialOptions.backgroundColor !== undefined
                ? partialOptions.backgroundColor
                : options.fillColor;
    ...
    const offsets = [
        { dx: 0, dy: 0, seedOffset: 1 },
        { dx: W * 0.5, dy: -H * 0.5, seedOffset: 2 },
        { dx: -W * 0.5, dy: H * 0.5, seedOffset: 3 },
        { dx: 0, dy: -H, seedOffset: 4 },
        { dx: -W, dy: 0, seedOffset: 5 },
    ];
    ...
    offsets.forEach(({ dx, dy, seedOffset }) => {
        ...
        const tile = buildTile({
            ...
            latBias: Math.max(0, Number.isFinite(options.lateralBias) ? options.lateralBias : 0),
