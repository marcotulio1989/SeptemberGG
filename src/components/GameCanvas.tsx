...
interface TilePatternVariant {
    texture: PIXI.Texture;
    width: number;
    height: number;
}

interface TilePatternCache {
    baseKey: string;
    baseSeedPosition: number;
    baseSeedDamage: number;
    variants: Map<number, TilePatternVariant>;
}

const MAX_TILE_PATTERN_VARIANTS = 96;
...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha }) => {
    ...
    const normalizedTilePatternKey = (() => {
        const normalized = {
            tileWidth: Math.max(16, Math.floor(tileGeneratorOptions.tileWidth)),
            tileHeight: Math.max(16, Math.floor(tileGeneratorOptions.tileHeight)),
            seedCount: Math.max(30, Math.floor(tileGeneratorOptions.seedCount)),
            damageProbability: clamp(tileGeneratorOptions.damageProbability, 0, 1),
            lateralFocus: clamp(tileGeneratorOptions.lateralFocus, 0, 1),
            lateralBias: clamp(tileGeneratorOptions.lateralBias, 0, 4),
            randomAmplitude: clamp(tileGeneratorOptions.randomAmplitude, 0, 1),
            outlineColor: tileGeneratorOptions.outlineColor,
            fillColor: tileGeneratorOptions.fillColor,
            crackColor: tileGeneratorOptions.crackColor,
            sideColor: tileGeneratorOptions.sideColor,
            thickness: Math.max(0, Math.floor(tileGeneratorOptions.thickness)),
            seedPosition: tileGeneratorOptions.seedPosition >>> 0,
            seedDamage: tileGeneratorOptions.seedDamage >>> 0,
        };
        return { normalized, key: JSON.stringify(normalized) };
    })();

    const tilePatternCache = useRef<TilePatternCache | null>(null);
    if (typeof document !== 'undefined') {
        const cache = tilePatternCache.current;
        if (!cache || cache.baseKey !== normalizedTilePatternKey.key) {
            if (cache) {
                cache.variants.forEach(entry => {
                    try { entry.texture.destroy(true); } catch (e) {}
                });
            }
            tilePatternCache.current = {
                baseKey: normalizedTilePatternKey.key,
                baseSeedPosition: normalizedTilePatternKey.normalized.seedPosition,
                baseSeedDamage: normalizedTilePatternKey.normalized.seedDamage,
                variants: new Map(),
            };
        } else {
            cache.baseSeedPosition = normalizedTilePatternKey.normalized.seedPosition;
            cache.baseSeedDamage = normalizedTilePatternKey.normalized.seedDamage;
        }
    }
    ...
    React.useEffect(() => () => {
        ...
        tilePatternCache.current = null;
    }, []);

    const requestTilePatternVariant = (variantSeed: number): TilePatternVariant | null => {
        if (typeof document === 'undefined') return null;
        const cache = tilePatternCache.current;
        if (!cache) return null;
        const normalizedSeed = (variantSeed >>> 0);
        let entry = cache.variants.get(normalizedSeed) || null;
        if (!entry) {
            try {
                const variantOptions: TileGeneratorOptions = {
                    ...tileGeneratorOptions,
                    seedPosition: (cache.baseSeedPosition ^ normalizedSeed) >>> 0,
                    seedDamage: (cache.baseSeedDamage ^ Math.imul(normalizedSeed, 7919)) >>> 0,
                };
                const canvas = generateIsometricTilePattern(variantOptions);
                ...
                entry = { texture, width: canvas.width, height: canvas.height };
                cache.variants.set(normalizedSeed, entry);
                while (cache.variants.size > MAX_TILE_PATTERN_VARIANTS) {
                    const oldestKey = cache.variants.keys().next().value;
                    if (oldestKey === undefined) break;
                    const oldest = cache.variants.get(oldestKey);
                    if (oldest) {
                        try { oldest.texture.destroy(true); } catch (e) {}
                    }
                    cache.variants.delete(oldestKey);
                }
            } catch (err) {
                return null;
            }
    ...
    // Desenhar quarteirões com esquinas curvas (integrado abaixo com polígonos reais)
        ...
        if (blockOutlines.current) {
            ...
            blockWorldPaths.forEach((worldPts: Point[]) => {
                ...
                const centroidDistanceWithinHeatmap = !!(
                    heatmapRadius !== null &&
                    centroidWorld &&
                    Math.hypot(centroidWorld.x - heatmapCenter.x, centroidWorld.y - heatmapCenter.y) <= heatmapRadius
                );
                const insideHeatmapRadius = centroidDistanceWithinHeatmap && !!tilePatternCache.current;
                const points = worldPts.map(p => worldToIso(p));
                if (points.length > 2) {
                    const useTex = !!(config as any).render.blockInteriorUseTexture && interiorTexture;
                    if (insideHeatmapRadius) {
                        try {
                            const variantSeed = centroidWorld
                                ? hashNumbers(
                                      normalizedTilePatternKey.normalized.seedPosition,
                                      normalizedTilePatternKey.normalized.seedDamage,
                                      centroidWorld.x,
                                      centroidWorld.y,
                                      worldPts.length,
                                  )
                            ...
                            const tilePatternInfo = requestTilePatternVariant(variantSeed);
                            if (!tilePatternInfo) {
                                ...
                                if (isoCentroid) {
                                    const offsetX = scaledWidth > 0
                                        ? ((isoCentroid.x % scaledWidth) + scaledWidth) % scaledWidth
                                        : 0;
                                    const offsetY = scaledHeight > 0
                                        ? ((isoCentroid.y % scaledHeight) + scaledHeight) % scaledHeight
                                        : 0;
