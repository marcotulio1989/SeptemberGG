...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha, sidewalkTexture, sidewalkScale, sidewalkAlpha, sidewalkTint }) => {
    ...
    const buildLoopMetrics = (points: Point[]): LoopMetrics | null => {
        ...
        for (let i = 0; i < loop.length; i++) {
            ...
            const dir = len > 1e-8 ? { x: dx / len, y: dy / len } : { x: 0, y: 0 };
            segmentDirs.push(dir);
            const normal = ccw ? { x: -dir.y, y: dir.x } : { x: dir.y, y: -dir.x };
            segmentNormals.push(normalizeVector(normal));
            total += len;
            cumulative.push(total);
        }
        const vertexNormals: Point[] = [];
        for (let i = 0; i < loop.length; i++) {
            const prev = segmentNormals[(i - 1 + segmentNormals.length) % segmentNormals.length];
            const curr = segmentNormals[i];
            let nx = prev.x + curr.x;
            let ny = prev.y + curr.y;
            if (!(Math.hypot(nx, ny) > 1e-8)) {
                nx = curr.x;
                ny = curr.y;
            }
            vertexNormals.push(normalizeVector({ x: nx, y: ny }));
        }
        return {
            points: loop.map(pt => ({ x: pt.x, y: pt.y })),
            cumulative,
            total,
            segmentDirs,
            segmentNormals,
    ...
    const sampleLoopFrame = (
        metrics: LoopMetrics,
        t: number
    ): { point: Point; normal: Point; tangent: Point } => {
        ...
        if (!(metrics.total > 1e-6)) {
            ...
            const normal = metrics.vertexNormals[0] || { x: 0, y: metrics.ccw ? 1 : -1 };
            const tangent = metrics.segmentDirs[0] || { x: 1, y: 0 };
            return {
        ...
        const localT = segLen > 1e-6 ? clamp((target - metrics.cumulative[segIndex]) / segLen, 0, 1) : 0;
        ...
        const tangent = normalizeVector(metrics.segmentDirs[segIndex] || { x: b.x - a.x, y: b.y - a.y });
        const startNormal = metrics.vertexNormals[segIndex] || { x: 0, y: metrics.ccw ? 1 : -1 };
        const endNormal = metrics.vertexNormals[(segIndex + 1) % metrics.vertexNormals.length] || startNormal;
        const blended = {
            x: startNormal.x * (1 - localT) + endNormal.x * localT,
            y: startNormal.y * (1 - localT) + endNormal.y * localT,
        };
        let normal = normalizeVector(blended);
        if (!(Math.hypot(normal.x, normal.y) > 1e-6)) {
            normal = normalizeVector(metrics.segmentNormals[segIndex] || tangent);
        }
    ...
    const renderSidewalkRing = ({
        ...
        try {
            ...
            if (innerPaths && innerPaths.length) {
                ...
                const gatherContours = (node: any) => {
                    ...
                    const outerCopy = orientedOuter.map(pt => ({ x: pt.x, y: pt.y }));
                    ...
                    ringContours.push({
                        outer: outerCopy.map(pt => ({ x: pt.x, y: pt.y })),
                        holes: holeLoops,
                    });
                    outerRings.push(outerCopy);
                };
        ...
        if (!outerRings.length && insetLoop && insetLoop.length >= 3) {
        ...
        const addRingSegments = (outerCandidate: Point[]) => {
            ...
            if (!outerMetrics || !(outerMetrics.total > 1e-4)) return;
            ...
            for (let ti = 0; ti < tileCount; ti++) {
                ...
                const frame0 = sampleLoopFrame(outerMetrics, t0);
                const frame1 = sampleLoopFrame(outerMetrics, t1);
                const outerStart = { x: frame0.point.x, y: frame0.point.y };
                const outerEnd = { x: frame1.point.x, y: frame1.point.y };
                const innerStart = {
                    x: outerStart.x + frame0.normal.x * widthM,
                    y: outerStart.y + frame0.normal.y * widthM,
                };
                const innerEnd = {
                    x: outerEnd.x + frame1.normal.x * widthM,
                    y: outerEnd.y + frame1.normal.y * widthM,
                };
