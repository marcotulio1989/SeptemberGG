...
const generateRoadCrackSprite = ({
    ...
    const feather = Math.max(1e-4, epsilonThreshold * 0.35);
    const hardThreshold = Math.max(1e-4, epsilonThreshold - feather);
    const thresholdRange = Math.max(1e-4, epsilonThreshold - hardThreshold);
        ...
        for (let px = 0; px < pixelCols; px++) {
            ...
            if (delta <= epsilonThreshold) {
                ...
                if (delta > hardThreshold) {
                    const denom = thresholdRange;
                    weight = Math.max(0, Math.min(1, (epsilonThreshold - delta) / denom));
                }
