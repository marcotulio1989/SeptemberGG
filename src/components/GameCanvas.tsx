...
const generateRoadCrackSprite = ({
    ...
    const stepIsoX = expandedSpanX / targetCols;
    const stepIsoY = expandedSpanY / targetRows;
    if (!(stepIsoX > 0) || !(stepIsoY > 0)) return null;

    const gridW = Math.max(2, targetCols + 1);
    const gridH = Math.max(2, targetRows + 1);
    ...
    const buffer = new Uint8Array(gridW * gridH * 4);
    const candidateBuf: number[] = [];
    let hits = 0;

    for (let jy = 0; jy < gridH; jy++) {
        const isoY = isoOriginY + (jy + 0.5) * stepIsoY;
        for (let ix = 0; ix < gridW; ix++) {
            const isoX = isoOriginX + (ix + 0.5) * stepIsoX;
    ...
    return {
        ...
        spriteX: isoOriginX,
        spriteY: isoOriginY,
        spriteWidth: gridW * stepIsoX,
        spriteHeight: gridH * stepIsoY,
    };
};
