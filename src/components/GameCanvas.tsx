...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha, sidewalkTexture, sidewalkScale, sidewalkAlpha, sidewalkTint }) => {
    ...
    const polygonSignedArea = (loop: Point[]): number => {
    ...
    const rotateLoopToClosestPoint = (loop: Point[], target: Point): Point[] => {
        if (!loop.length) return loop.slice();
        let bestIdx = 0;
        let bestDist = Infinity;
        for (let i = 0; i < loop.length; i++) {
            const d = sqrDist(loop[i], target);
            if (d < bestDist) {
                bestDist = d;
                bestIdx = i;
            }
        }
        if (bestIdx === 0) return loop.slice();
        const rotated: Point[] = [];
        for (let i = 0; i < loop.length; i++) {
            rotated.push(loop[(bestIdx + i) % loop.length]);
        }
        return rotated;
    };

    type ArcLengthData = { cumulative: number[]; total: number };

    const buildArcLengthData = (loop: Point[]): ArcLengthData => {
    ...
    const sampleLoopAtNormalized = (loop: Point[], arc: ArcLengthData, t: number): Point => {
        if (!loop.length) return { x: 0, y: 0 };
        if (arc.total <= 1e-6 || arc.cumulative.length < 2) {
            const p = loop[Math.max(0, Math.min(loop.length - 1, Math.round(((t % 1) + 1) % 1 * (loop.length - 1))))];
            return { x: p.x, y: p.y };
        }
        let tn = t;
        if (!Number.isFinite(tn)) tn = 0;
        tn = tn - Math.floor(tn);
        const dist = tn * arc.total;
        for (let i = 0; i < loop.length; i++) {
            const start = arc.cumulative[i];
            const end = arc.cumulative[i + 1];
            const segLen = end - start;
            if (segLen <= 1e-6) continue;
            if (dist <= end || i === loop.length - 1) {
                const local = clamp((dist - start) / segLen, 0, 1);
                const a = loop[i];
                const b = loop[(i + 1) % loop.length];
                return {
                    x: a.x + (b.x - a.x) * local,
    ...
    const renderSidewalkRing = ({
        ...
        let innerLoopFromClipper: Point[] | null = null;
        ...
        if (innerPaths && innerPaths.length) {
            ...
            for (const path of innerPaths) {
                ...
                const loop = path.map((p: any) => ({ x: p.X / clipScale, y: p.Y / clipScale }));
                const area = Math.abs(polygonSignedArea(loop));
                if (area > bestArea) {
                    bestArea = area;
                    innerLoopFromClipper = loop;
                }
            }
        }

        let insetLoop = innerLoopFromClipper;
        if (!insetLoop || insetLoop.length < 3) {
            insetLoop = insetPolygonByDistance(outerLoop, widthM);
        }

        if (insetLoop && insetLoop.length >= 3) {
            const outerArea = polygonSignedArea(outerLoop);
            const innerArea = polygonSignedArea(insetLoop);
            if (outerArea * innerArea < 0) {
                insetLoop = insetLoop.slice().reverse();
            }
            insetLoop = rotateLoopToClosestPoint(insetLoop, outerLoop[0]);

            const outerArc = buildArcLengthData(outerLoop);
            const innerArc = buildArcLengthData(insetLoop);
            ...
            if (outerLoop.length === outerArc.cumulative.length - 1 && innerArc.total > 1e-6 && outerArc.total > 1e-6) {
                for (let i = 0; i < outerLoop.length; i++) {
                    const a = outerLoop[i];
                    const b = outerLoop[(i + 1) % outerLoop.length];
                    const segStart = outerArc.cumulative[i];
                    const segEnd = outerArc.cumulative[i + 1];
                    const segLength = segEnd - segStart;
                    if (!(segLength > 1e-4)) continue;

                    const tileCount = Math.max(1, Math.round(segLength / tileLengthM));
                    for (let t = 0; t < tileCount; t++) {
                        ...
                        const normStart = (segStart + segLength * t0) / outerArc.total;
                        const normEnd = (segStart + segLength * t1) / outerArc.total;
                        const innerStart = sampleLoopAtNormalized(insetLoop!, innerArc, normStart);
                        const innerEnd = sampleLoopAtNormalized(insetLoop!, innerArc, normEnd);
                        walkwayQuads.push([outerStart, outerEnd, innerEnd, innerStart]);
        ...
        if (!walkwayQuads.length && clipperRingPaths && clipperRingPaths.length) {
            ringWorldPolys.forEach(poly => walkwayQuads.push(poly));
        }
