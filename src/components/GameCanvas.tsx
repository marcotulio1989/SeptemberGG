...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha, sidewalkTexture, sidewalkScale, sidewalkAlpha, sidewalkTint }) => {
    ...
    const normalizePoint = (v: Point): Point => {
    ...
    const sampleLoopFrame = (
        ...
        if (!metrics.points.length) {
            ...
            const fallbackTangent = { x: 1, y: 0 };
            const fallbackNormal = ccw ? { x: 0, y: 1 } : { x: 0, y: -1 };
            return { point: fallbackPoint, tangent: fallbackTangent, normal: fallbackNormal };
        }

        const n = metrics.points.length;
        const total = metrics.total;
        if (!(total > 1e-6)) {
            const first = metrics.points[0];
            const tangent = { x: 1, y: 0 };
            const normal = ccw ? { x: 0, y: 1 } : { x: 0, y: -1 };
            return { point: { x: first.x, y: first.y }, tangent, normal };
        }
        ...
        while (segIndex < n) {
            const nextVal = metrics.cumulative[segIndex + 1];
            if (target <= nextVal || segIndex === n - 1) break;
            segIndex++;
        }

        const segLen = metrics.cumulative[segIndex + 1] - metrics.cumulative[segIndex];
        const a = metrics.points[segIndex % n];
        const b = metrics.points[(segIndex + 1) % n];
        let localT = 0;
        if (segLen > 1e-6) {
            localT = (target - metrics.cumulative[segIndex]) / segLen;
        }
        const point = {
            x: a.x + (b.x - a.x) * localT,
            y: a.y + (b.y - a.y) * localT,
        ...
        let tangent = { x: b.x - a.x, y: b.y - a.y };
        let tangentLen = Math.hypot(tangent.x, tangent.y);
        if (!(tangentLen > 1e-6)) {
            const prev = metrics.points[(segIndex - 1 + n) % n];
            const next = metrics.points[(segIndex + 2) % n];
            tangent = { x: next.x - prev.x, y: next.y - prev.y };
            tangentLen = Math.hypot(tangent.x, tangent.y);
        }
        if (!(tangentLen > 1e-6)) {
            tangent = { x: 1, y: 0 };
        } else {
    ...
    const intersectRayWithSegment = (
        ...
        const t = (diff.x * seg.y - diff.y * seg.x) / cross;
    ...
    const intersectRayWithLoop = (origin: Point, dir: Point, loop: Point[]): Point | null => {
        const dirNorm = normalizePoint(dir);
        if (Math.abs(dirNorm.x) < 1e-6 && Math.abs(dirNorm.y) < 1e-6) return null;
        ...
        for (let i = 0; i < loop.length; i++) {
            const a = loop[i];
            const b = loop[(i + 1) % loop.length];
            const intersection = intersectRayWithSegment(origin, dirNorm, a, b);
            if (!intersection) continue;
            if (intersection.distance < bestDistance && intersection.distance > 1e-6) {
    ...
    const renderSidewalkRing = ({
        ...
        const addRingSegments = (outerCandidate: Point[], innerCandidate: Point[]) => {
            ...
            for (let ti = 0; ti < tileCount; ti++) {
                ...
                const frameStart = sampleLoopFrame(outerMetrics, t0, true);
                const frameEnd = sampleLoopFrame(outerMetrics, t1, true);
                const outerStart = frameStart.point;
                const outerEnd = frameEnd.point;
                const innerStartRay = intersectRayWithLoop(outerStart, frameStart.normal, alignedInner);
                const innerEndRay = intersectRayWithLoop(outerEnd, frameEnd.normal, alignedInner);
                const innerStart = innerStartRay || sampleLoopPoint(innerMetrics, t0);
                const innerEnd = innerEndRay || sampleLoopPoint(innerMetrics, t1);
                ...
                tilePolys.push([
                    { x: outerStart.x, y: outerStart.y },
                    { x: outerEnd.x, y: outerEnd.y },
                    { x: innerEnd.x, y: innerEnd.y },
                    { x: innerStart.x, y: innerStart.y },
                ]);
            }
        };
