...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha, sidewalkTexture, sidewalkScale, sidewalkAlpha, sidewalkTint }) => {
    ...
    const buildLoopMetrics = (points: Point[]): LoopMetrics | null => {
        ...
        return { points: loop.map(pt => ({ x: pt.x, y: pt.y })), cumulative, total, orientation };
    };
    ...
    const sampleLoopInfo = (
        metrics: LoopMetrics,
        t: number
    ): { point: Point; tangent: Point; normal: Point } => {
        const point = sampleLoopPoint(metrics, t);
        ...
        const perimeter = metrics.total;
        const delta = perimeter > 1e-4 ? Math.min(0.25, 1 / Math.max(3, metrics.points.length)) / 2 : 0;
        const prev = sampleLoopPoint(metrics, t - delta);
        const next = sampleLoopPoint(metrics, t + delta);
        let dx = next.x - prev.x;
        let dy = next.y - prev.y;
        let len = Math.hypot(dx, dy);
        if (!(len > 1e-6)) {
            const segDelta = 1 / Math.max(3, metrics.points.length);
            const altPrev = sampleLoopPoint(metrics, t - segDelta);
            const altNext = sampleLoopPoint(metrics, t + segDelta);
            dx = altNext.x - altPrev.x;
            dy = altNext.y - altPrev.y;
        ...
        const orientation = metrics.orientation >= 0 ? 1 : -1;
        const normal =
            orientation >= 0
    ...
    const intersectRaySegment = (
        ...
        a: Point,
        b: Point
    ): { point: Point; tRay: number } | null => {
        const seg = { x: b.x - a.x, y: b.y - a.y };
        const denom = dir.x * seg.y - dir.y * seg.x;
        if (Math.abs(denom) < 1e-8) return null;
        const w = { x: a.x - origin.x, y: a.y - origin.y };
        const tRay = (w.x * seg.y - w.y * seg.x) / denom;
        const u = (w.x * dir.y - w.y * dir.x) / denom;
        if (tRay >= -1e-6 && u >= -1e-6 && u <= 1 + 1e-6) {
            return {
                point: { x: origin.x + dir.x * tRay, y: origin.y + dir.y * tRay },
                tRay,
            };
        }
        return null;
    ...
    const projectPointToInner = (
        origin: Point,
        inward: Point,
        innerLoop: Point[],
        maxDistance: number
    ): Point | null => {
        const dirLen = Math.hypot(inward.x, inward.y);
        if (!(dirLen > 1e-6)) return null;
        const dir = { x: inward.x / dirLen, y: inward.y / dirLen };
        let best: { point: Point; tRay: number } | null = null;
        for (let i = 0; i < innerLoop.length; i++) {
            const a = innerLoop[i];
            const b = innerLoop[(i + 1) % innerLoop.length];
            const hit = intersectRaySegment(origin, dir, a, b);
            if (!hit) continue;
            if (hit.tRay < -1e-6) continue;
            if (!(hit.tRay <= maxDistance + 1e-3)) continue;
            if (!best || hit.tRay < best.tRay) {
                best = hit;
            }
        }
    ...
    const renderSidewalkRing = ({
        ...
        try {
            ...
            if (innerPaths && innerPaths.length) {
                ...
                const gatherRings = (node: any) => {
                    if (!node) return;
                    const contour = node.Contour && node.Contour();
                    const childs = node.Childs && node.Childs();
                    if (contour && contour.length > 2 && childs && childs.length) {
                        const outerWorld = ensureLoopOrientation(
                        ...
                        for (let i = 0; i < childs.length; i++) {
                            ...
                            if (holeContour && holeContour.length > 2) {
                                ...
                                const alignedInner = rotateLoopToClosestPoint(innerWorld, outerWorld[0]);
                                ringsForTiles.push({
                                    outer: outerWorld.map(pt => ({ x: pt.x, y: pt.y })),
                                    inner: alignedInner,
                                });
                            }
                            gatherRings(child);
        ...
        if (!ringsForTiles.length && insetLoop && insetLoop.length >= 3) {
            ringsForTiles.push({
                outer: outerLoop.map(pt => ({ x: pt.x, y: pt.y })),
                inner: ensureLoopOrientation(insetLoop, true),
            });
        }
        ...
        const addRingSegments = (outerCandidate: Point[], innerCandidate: Point[]) => {
            ...
            const tileCount = Math.max(1, Math.round(outerMetrics.total / tileLengthM));
            for (let ti = 0; ti < tileCount; ti++) {
                const t0 = ti / tileCount;
                const t1 = (ti + 1) / tileCount;
                const startInfo = sampleLoopInfo(outerMetrics, t0);
                const endInfo = sampleLoopInfo(outerMetrics, t1);
                const maxProject = widthM * 2;
                let innerStart = projectPointToInner(startInfo.point, startInfo.normal, alignedInner, maxProject);
                let innerEnd = projectPointToInner(endInfo.point, endInfo.normal, alignedInner, maxProject);
                if (!innerStart) {
                    innerStart = sampleLoopPoint(innerMetrics, t0);
                }
                if (!innerEnd) {
                    innerEnd = sampleLoopPoint(innerMetrics, t1);
                }
                if (!innerStart || !innerEnd) continue;
                walkwayQuads.push([startInfo.point, endInfo.point, innerEnd, innerStart]);
                tilePolys.push([startInfo.point, endInfo.point, innerEnd, innerStart]);
            }
        };
        ...
        const drawIsoPolygon = (pts: Point[]): void => {
            ...
            if (isoPts.length > 2) {
                ringG.drawPolygon(isoPts.flatMap(pt => [pt.x, pt.y]));
            }
        };
        ...
        const drawPolyNode = (node: any): void => {
            ...
            const contour = node.Contour && node.Contour();
            if (contour && contour.length > 2) {
                const isoCoords: number[] = [];
                for (let i = 0; i < contour.length; i++) {
                    const p = contour[i];
                    const iso = worldToIso({ x: p.X / clipScale, y: p.Y / clipScale });
                    isoCoords.push(iso.x, iso.y);
                }
                if (isoCoords.length >= 6) {
                    if (typeof node.IsHole === 'function' && node.IsHole()) {
                        ringG.beginHole();
                        ringG.drawPolygon(isoCoords);
                        ringG.endHole();
                    } else {
                        ringG.drawPolygon(isoCoords);
                    }
        ...
        } else if (clipperRingPaths && clipperRingPaths.length) {
