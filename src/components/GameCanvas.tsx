...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha, sidewalkTexture, sidewalkScale, sidewalkAlpha, sidewalkTint }) => {
    ...
    const renderSidewalkRing = ({
        ...
        if (outerLoop.length >= 3) {
            ...
            let innerPaths: any = null;

            try {
                const offset = new ClipperLib.ClipperOffset();
                offset.AddPath(outerPath, ClipperLib.JoinType.jtMiter, ClipperLib.EndType.etClosedPolygon);
                const generatedInner = new ClipperLib.Paths();
                offset.Execute(generatedInner, -widthM * clipScale);
                if (generatedInner && generatedInner.length) {
                    innerPaths = generatedInner;
                }
            } catch {
                innerPaths = null;
            }

            if ((!innerPaths || !innerPaths.length) && insetLoop && insetLoop.length >= 3) {
                innerPaths = [
                    insetLoop.map(p => ({ X: Math.round(p.x * clipScale), Y: Math.round(p.y * clipScale) })),
                ];
            }
            ...
            if (innerPaths && innerPaths.length) {
                try {
                    ...
                    clipperRingPaths = null;
                    clipperRingPolyTree = null;
                }
            }
        }
