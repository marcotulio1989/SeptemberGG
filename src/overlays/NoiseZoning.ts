...
const NoiseZoning: InternalNoiseZoning = {
  ...
  // pixelated mode: no large ImageData allocation
    ...
    const mapToCoarseCoord = (edges: Float32Array, count: number, value: number) => {
      ...
      const idx = lo - 1;
      if (idx < 0) return 0;
      const startEdge = edges[idx];
      const nextEdge = edges[idx + 1];
      const span = nextEdge - startEdge;
      if (!(span > 1e-6)) return idx;
      const frac = (value - startEdge) / span;
      if (!Number.isFinite(frac)) return idx;
      const clamped = Math.min(1, Math.max(0, frac));
      return idx + clamped;
    };
    const toCoarsePoint = (screenPt: { x: number; y: number }) => ({
      x: mapToCoarseCoord(xEdges, coarseW, screenPt.x),
      y: mapToCoarseCoord(yEdges, coarseH, screenPt.y),
    });
    ...
    try {
      ...
      if (
        ...
        for (const seg of roadSegments) {
          ...
          const a = toCoarsePoint(A);
          const b = toCoarsePoint(B);
    ...
    try {
      ...
      for (let gy = 0; gy < coarseH; gy++) {
        const y0 = Math.floor(yEdges[Math.max(gy, 0)]);
        const y1 = Math.ceil(yEdges[Math.min(gy + 1, yEdges.length - 1)]);
        const clampedY0 = Math.max(0, Math.min(h, y0));
        const clampedY1 = Math.max(0, Math.min(h, y1));
        const hPx = clampedY1 - clampedY0;
        if (!(hPx > 0)) continue;
        for (let gx = 0; gx < coarseW; gx++) {
          ...
          const x0 = Math.floor(xEdges[Math.max(gx, 0)]);
          const x1 = Math.ceil(xEdges[Math.min(gx + 1, xEdges.length - 1)]);
          const clampedX0 = Math.max(0, Math.min(w, x0));
          const clampedX1 = Math.max(0, Math.min(w, x1));
          const wPx = clampedX1 - clampedX0;
          if (!(wPx > 0)) continue;
          // draw block
          this._ctx.fillRect(clampedX0, clampedY0, wPx, hPx);
        }
      }
