...
const generateRoadCrackContours = ({
    ...
    const stepIsoX = Math.max(1, expandedSpanX / targetCols);
    const stepIsoY = Math.max(1, expandedSpanY / targetRows);
    if (!(stepIsoX > 0) || !(stepIsoY > 0)) return [];

    const gridW = Math.max(2, Math.ceil(expandedSpanX / stepIsoX) + 1);
    const gridH = Math.max(2, Math.ceil(expandedSpanY / stepIsoY) + 1);
    if (gridW < 2 || gridH < 2) return [];

    const isoOriginX = expandedMinX;
    const isoOriginY = expandedMinY;
    const spanForBucketsX = stepIsoX * Math.max(1, gridW - 1);
    const spanForBucketsY = stepIsoY * Math.max(1, gridH - 1);
    const epsilonIso = Math.max(0.01, epsilonPx);
    ...
    let attempts = 0;
    while (isoSeeds.length < targetSeeds * 2 && attempts < maxAttempts) {
        attempts++;
    ...
    const actualSeeds = isoSeeds.length / 2;
    ...
    const cellIsoX = Math.max(stepIsoX, spanForBucketsX / bucketCols, 1e-4);
    const cellIsoY = Math.max(stepIsoY, spanForBucketsY / bucketRows, 1e-4);
    ...
    for (let jy = 0; jy < gridH; jy++) {
        const isoY = isoOriginY + jy * stepIsoY;
        for (let ix = 0; ix < gridW; ix++) {
            const isoX = isoOriginX + ix * stepIsoX;
            ...
            for (let k = 0; k < source.length; k++) {
                const idx = source[k];
                const dx = isoX - isoSeeds[2 * idx];
                const dy = isoY - isoSeeds[2 * idx + 1];
            ...
            mask[jy * gridW + ix] = delta < epsilonIso ? 1 : 0;
        }
    }

    const isoContours = marchingSquaresContoursLocal(mask, gridW, gridH, stepIsoX, stepIsoY, isoOriginX, isoOriginY);
    ...
    const drawCrackedRoads = (segments: Segment[]) => {
        ...
        segments.forEach((segment, segmentIndex) => {
            ...
            intervals.forEach((interval, intervalIndex) => {
                ...
                if (seeds < 2) return;
                const intervalEndWorld = { x: baseX + ux * intervalLen, y: baseY + uy * intervalLen };
                const isoStart = worldToIso({ x: baseX, y: baseY });
                const isoEnd = worldToIso(intervalEndWorld);
                const isoLength = Number.isFinite(isoStart?.x) && Number.isFinite(isoStart?.y) && Number.isFinite(isoEnd?.x) && Number.isFinite(isoEnd?.y)
                    ? Math.hypot(isoEnd.x - isoStart.x, isoEnd.y - isoStart.y)
                    : intervalLen;
                const halfWidth = roadWidth * 0.5;
                const centerWorld = { x: baseX + ux * intervalLen * 0.5, y: baseY + uy * intervalLen * 0.5 };
                const isoLeft = worldToIso({ x: centerWorld.x + nx * halfWidth, y: centerWorld.y + ny * halfWidth });
                const isoRight = worldToIso({ x: centerWorld.x - nx * halfWidth, y: centerWorld.y - ny * halfWidth });
                const isoWidth = Number.isFinite(isoLeft?.x) && Number.isFinite(isoLeft?.y) && Number.isFinite(isoRight?.x) && Number.isFinite(isoRight?.y)
                    ? Math.hypot(isoLeft.x - isoRight.x, isoLeft.y - isoRight.y)
                    : roadWidth;
                let samplesU = Math.max(2, Math.round(Math.max(isoLength, 1) * segSampleAlong));
                let samplesV = Math.max(2, Math.round(Math.max(isoWidth, 1) * segSampleAcross));
