...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha, sidewalkTexture, sidewalkScale, sidewalkAlpha, sidewalkTint }) => {
    ...
    const intersectSegments = (
        ...
        const qpX = q1.x - p1.x;
        const qpY = q1.y - p1.y;
        const t = (qpX * s.y - qpY * s.x) / denom;
        const u = (qpX * r.y - qpY * r.x) / denom;
        if (Number.isFinite(t) && Number.isFinite(u) && t >= -1e-6 && t <= 1 + 1e-6 && u >= -1e-6 && u <= 1 + 1e-6) {
            return {
                x: p1.x + r.x * t,
                y: p1.y + r.y * t,
            };
    ...
    const renderSidewalkRing = ({
        ...
        if (outerLoop.length >= 3) {
            ...
            try {
                if (typeof coSide.MiterLimit === 'number') {
                    coSide.MiterLimit = Math.max(1, Math.min(10, (config as any).render?.sidewalkMiterLimit ?? 2));
                }
                if (typeof coSide.ArcTolerance === 'number') {
                    const tol = Math.max(0.1, Math.min(clipScale * 0.5, (config as any).render?.sidewalkArcTolerance ?? 0.25));
                    coSide.ArcTolerance = tol;
                }
            } catch (_) { }
            ...
            if (innerPaths && innerPaths.length) {
                ...
                if (ringPaths.length) {
                    clipperRingPaths = ringPaths;
                    ringWorldPolys = ringPaths.map((path: any) => path.map((p: any) => ({ x: p.X / clipScale, y: p.Y / clipScale })));
                }
                const innerLoopsWorld: Point[][] = innerPaths.map((path: any) => path.map((p: any) => ({ x: p.X / clipScale, y: p.Y / clipScale })));
                if (innerLoopsWorld.length) {
                    let bestArea = -Infinity;
                    for (const loop of innerLoopsWorld) {
                        const area = Math.abs(polygonArea(loop));
                        if (area > bestArea) {
                            bestArea = area;
                            primaryInnerLoop = loop;
                        }
                    }
                }
            }
        ...
        const fallbackInnerPoint = (outerPoint: Point): Point => {
            if (!centroid) return { x: outerPoint.x, y: outerPoint.y };
            const dirX = centroid.x - outerPoint.x;
            const dirY = centroid.y - outerPoint.y;
            const len = Math.hypot(dirX, dirY);
            if (!(len > 1e-6)) {
                return { x: outerPoint.x, y: outerPoint.y };
            }
            const dist = Math.min(widthM * 0.95, len * 0.8);
            return {
                x: outerPoint.x + (dirX / len) * dist,
                y: outerPoint.y + (dirY / len) * dist,
            };
        };

        const findInnerPoint = (outerPoint: Point): Point | null => {
            if (!primaryInnerLoop || primaryInnerLoop.length < 2) return null;
            const targets: Point[] = [];
            if (centroid) targets.push({ x: centroid.x, y: centroid.y });
            if (centroid) {
                const dirX = centroid.x - outerPoint.x;
                const dirY = centroid.y - outerPoint.y;
                const len = Math.hypot(dirX, dirY) || 1;
                targets.push({
                    x: outerPoint.x + (dirX / len) * Math.max(widthM * 5, len * 2),
                    y: outerPoint.y + (dirY / len) * Math.max(widthM * 5, len * 2),
                });
            }
            let best: Point | null = null;
            let bestDist = Infinity;
            for (const target of targets) {
                for (let i = 0; i < primaryInnerLoop.length; i++) {
                    const ia = primaryInnerLoop[i];
                    const ib = primaryInnerLoop[(i + 1) % primaryInnerLoop.length];
                    const inter = intersectSegments(outerPoint, target, ia, ib);
                    if (inter) {
                        const d = Math.hypot(inter.x - outerPoint.x, inter.y - outerPoint.y);
                        if (d < bestDist) {
                            bestDist = d;
                            best = inter;
                        }
        ...
        if (insetLoop && insetLoop.length === outerLoop.length) {
            ...
            for (let i = 0; i < outerLoop.length; i++) {
                const a = outerLoop[i];
                const b = outerLoop[(i + 1) % outerLoop.length];
                const ai = insetLoop[i];
                const bi = insetLoop[(i + 1) % insetLoop.length];
                const edgeLen = Math.hypot(b.x - a.x, b.y - a.y);
                const innerLen = Math.hypot(bi.x - ai.x, bi.y - ai.y);
                if (!(edgeLen > 1e-4) || !(innerLen > 1e-4)) {
                    continue;
                }

                walkwayQuads.push([a, b, bi, ai]);

                const tileCount = Math.max(1, Math.round(edgeLen / tileLengthM));
            ...
            for (let i = 0; i < outerLoop.length; i++) {
                const a = outerLoop[i];
                const b = outerLoop[(i + 1) % outerLoop.length];
                const innerA = findInnerPoint(a) ?? fallbackInnerPoint(a);
                const innerB = findInnerPoint(b) ?? fallbackInnerPoint(b);
                const edgeLen = Math.hypot(b.x - a.x, b.y - a.y);
                if (!(edgeLen > 1e-4)) continue;
                walkwayQuads.push([a, b, innerB, innerA]);
                ...
                for (let t = 0; t < tileCount; t++) {
                    ...
                    const innerStart = findInnerPoint(outerStart) ?? fallbackInnerPoint(outerStart);
                    const innerEnd = findInnerPoint(outerEnd) ?? fallbackInnerPoint(outerEnd);
                    tilePolys.push([outerStart, outerEnd, innerEnd, innerStart]);
                }
            }
        }

        if (!walkwayQuads.length && clipperRingPaths && clipperRingPaths.length) {
            walkwayQuads.push(...ringWorldPolys);
        }
        if (!walkwayQuads.length && !(clipperRingPaths && clipperRingPaths.length)) {
            return;
        }
        ...
        if (useSideTex) {
            ...
            try {
                ...
                const s = pxPerDesired / Math.max(8, selTilePx);
                const A = (config as any).render.isoA ?? 1;
                const B = (config as any).render.isoB ?? 0;
                const C = (config as any).render.isoC ?? 0;
                const D = (config as any).render.isoD ?? 1;
        ...
        const fillPolys = (ringWorldPolys && ringWorldPolys.length) ? ringWorldPolys : walkwayQuads;
        for (const poly of fillPolys) {
            const pts = poly.map(p => worldToIso(p));
            if (pts.length > 2) {
                ringG.drawPolygon(pts.flatMap(pt => [pt.x, pt.y]));
            }
        }
