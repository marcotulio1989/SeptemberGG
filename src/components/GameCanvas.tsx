...
const generateVoronoiContours = (
    ...
    const metricScale = (() => {
        const scale = options?.metricScale;
        if (!Number.isFinite(scale) || !scale) return 1;
        return scale;
    })();
    ...
    const drawCrackedRoads = (segments: Segment[]) => {
        ...
        segments.forEach((segment, segmentIndex) => {
            ...
            intervals.forEach((interval, intervalIndex) => {
                ...
                const intervalSpanT = endT - startT;
                const desiredPatchLen = Math.max(roadWidth * 1.2, 3.2);
                let patchCount = Math.max(1, Math.round(intervalLen / desiredPatchLen));
                patchCount = Math.min(patchCount, 48);
                const patchStepT = intervalSpanT / patchCount;

                for (let patchIndex = 0; patchIndex < patchCount; patchIndex++) {
                    const patchStartT = startT + patchStepT * patchIndex;
                    const patchEndT = patchIndex === patchCount - 1 ? endT : patchStartT + patchStepT;
                    if (!(patchEndT > patchStartT + 1e-4)) continue;
                    const patchLen = segLen * (patchEndT - patchStartT);
                    if (patchLen < roadWidth * 0.3) continue;

                    const area = patchLen * roadWidth;
                    let seeds = Math.max(8, Math.round(area * segSeedDensity));
                    seeds = Math.min(seeds, segMaxSeeds);
                    if (seeds < 2) continue;

                    let samplesU = Math.max(2, Math.round(patchLen * segSampleAlong));
                    ...
                    if (samplesU < 2 || samplesV < 2) continue;

                    const patchStartOffset = segLen * patchStartT;
                    const baseX = start.x + ux * patchStartOffset;
                    const baseY = start.y + uy * patchStartOffset;
                    ...
                    const longestDim = Math.max(patchLen, roadWidth, 1);
                    const metricScale = 1 / longestDim;
                    ...
                    const minContourLen = Math.max(Math.min(patchLen, roadWidth) * 0.35, 2.0);
