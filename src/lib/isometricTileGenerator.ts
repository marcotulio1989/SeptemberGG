...
export interface TileGeneratorOptions {
    ...
    seedPosition: number;
    seedDamage: number;
    variantGridColumns: number;
    variantGridRows: number;
}

export interface TilePatternAtlas {
...
const scrambleSeed = (base: number, salt: number): number => {
    let h = (base >>> 0) ^ 0x9e3779b9;
    h = Math.imul(h ^ (h >>> 16) ^ (salt >>> 0), 0x85ebca6b) >>> 0;
    h ^= h >>> 13;
...
export const generateIsometricTilePattern = (
    ...
    const tileCanvasHeight = H + thickness;
    const rngPosBase = createPRNG(options.seedPosition >>> 0);
    const rngDmgBase = createPRNG(options.seedDamage >>> 0);
    const columns = clampInt(options.variantGridColumns ?? defaultOptions.variantGridColumns, 1, 24);
    const rows = clampInt(options.variantGridRows ?? defaultOptions.variantGridRows, 1, 24);

    const patternCanvas = document.createElement('canvas');
    patternCanvas.width = W * columns;
    patternCanvas.height = tileCanvasHeight * rows;
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < columns; col++) {
            variantIndex++;
            const posSeed = scrambleSeed(options.seedPosition, variantIndex + Math.floor(rngPosBase() * 0xffffffff));
            const dmgSeed = scrambleSeed(options.seedDamage, variantIndex + Math.floor(rngDmgBase() * 0xffffffff) + 7919);
            const tile = buildTile({
                W,
                H,
                count: seedCount,
                damageProb: clampUnit(options.damageProbability),
                rngPos: createPRNG(posSeed >>> 0),
                rngDmg: createPRNG(dmgSeed >>> 0),
