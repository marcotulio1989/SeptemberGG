...
import { generateIsometricTilePattern, getDefaultTileOptions, TileGeneratorOptions } from '../lib/isometricTileGenerator';
...
const toPositiveInt = (value: unknown, fallback: number, min: number) => {
...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha }) => {
    ...
    const centralTilePatternKey = useRef<string | null>(null);
    if (typeof document !== 'undefined') {
        const renderCfg = ((config as any).render || {}) as Record<string, unknown>;
        const tileCfg = (renderCfg.blockInteriorCentralTile || {}) as Record<string, unknown>;
        const enabled = tileCfg.enabled !== false;
        const baseOptions = getDefaultTileOptions();
        if (!enabled) {
            if (centralTilePattern.current) {
                try { centralTilePattern.current.texture.destroy(true); } catch (e) {}
            }
            centralTilePattern.current = null;
            centralTilePatternKey.current = 'disabled';
        } else {
            const tileOptions: Partial<TileGeneratorOptions> = {
                tileWidth: toPositiveInt(tileCfg.tileWidth, baseOptions.tileWidth, 16),
                tileHeight: toPositiveInt(tileCfg.tileHeight, baseOptions.tileHeight, 16),
                seedCount: toPositiveInt(tileCfg.seedCount, baseOptions.seedCount, 30),
                thickness: Math.max(0, Math.floor(toFiniteNumber(tileCfg.thickness, baseOptions.thickness))),
                damageProbability: clamp01(toFiniteNumber(tileCfg.damageProbability, baseOptions.damageProbability)),
                lateralFocus: clamp01(toFiniteNumber(tileCfg.lateralFocus, baseOptions.lateralFocus)),
                lateralBias: clamp01(toFiniteNumber(tileCfg.lateralBias, baseOptions.lateralBias)),
                randomAmplitude: clamp01(toFiniteNumber(tileCfg.randomAmplitude, baseOptions.randomAmplitude)),
                outlineColor: toHexColor(tileCfg.outlineColor, baseOptions.outlineColor),
                fillColor: toHexColor(tileCfg.fillColor, baseOptions.fillColor),
                crackColor: toHexColor(tileCfg.crackColor, baseOptions.crackColor),
                sideColor: toHexColor(tileCfg.sideColor, baseOptions.sideColor),
                backgroundColor: toHexColor(tileCfg.backgroundColor, baseOptions.backgroundColor),
                seedPosition: Math.floor(toFiniteNumber(tileCfg.seedPosition, baseOptions.seedPosition)),
                seedDamage: Math.floor(toFiniteNumber(tileCfg.seedDamage, baseOptions.seedDamage)),
            };
            const patternScale = clamp(Math.abs(toFiniteNumber(tileCfg.scale, 1)), 0.05, 20);
            const patternAlpha = clamp01(toFiniteNumber(tileCfg.alpha, 1));
            const patternKey = JSON.stringify({
                ...tileOptions,
                scale: patternScale,
                alpha: patternAlpha,
            });
            if (centralTilePatternKey.current !== patternKey) {
                try {
                    const canvas = generateIsometricTilePattern(tileOptions);
                    if (canvas) {
                        ...
                        centralTilePatternKey.current = patternKey;
                    } else {
                        centralTilePattern.current = null;
                        centralTilePatternKey.current = null;
                    }
                } catch (e) {
                    centralTilePattern.current = null;
                    centralTilePatternKey.current = null;
            ...
            blockWorldPaths.forEach((worldPts: Point[]) => {
                ...
                if (points.length > 2) {
                    ...
                    if (insideHeatmapRadius) {
                        try {
                            const { texture, width, height, scale, alpha } = tilePatternInfo!;
                            const matrix = new PIXI.Matrix();
                            ...
                            if (Math.abs(effectiveScale - 1) > 1e-4) matrix.scale(effectiveScale, effectiveScale);
                            if (isoCentroid) {
                            ...
                            gInner.beginTextureFill({ texture, alpha: clamp01(alpha), matrix });
